...
<div class="container mt-4">
    <div class="d-flex justify-content-between align-items-center mb-3">
        ...
        @if (auth()->check() && in_array(auth()->user()->role, [\App\Enums\UserRoleEnum::Administrator, \App\Enums\UserRoleEnum::Author]))
...
@endif
                        ...
                        <div class="card-footer text-muted d-flex flex-column">
                            ...
                            <small class="text-light">Stok: {{ $product->stock }}</small>
                            ...
                            {{-- Tombol edit (hanya admin dan author) --}}
                            @if (auth()->check() && in_array(auth()->user()->role, [\App\Enums\UserRoleEnum::Administrator, \App\Enums\UserRoleEnum::Author]))
                                <a href="{{ route('products.edit', $product->id) }}" class="btn btn-sm btn-warning mt-2">
                                    ✏️ Edit Produk
                                </a>
                            @endif
                        </div>
